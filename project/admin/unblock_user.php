...
<html>
 ...
 <li class="active"><a href="unblock_user.php">Unblock User</a></li>
 ...
 <div class="col-sm-8"> 
  ...
  <select class="form-control" name="user">
   <option value="Select User">Select User </option>
	...
	{
		//for displaying block_user to admin
	 require "../makeconnection.php";
	$result=$con->query("select * from tbl_signup where role=0 and block=0");
  ...
  <br/>
  <center>
   <input type="submit" class="btn btn-success" value="Unblock" name="unblock_btn"> 
  </center>
  </form>
  <br></div>
  </div>
...
if(isset($_POST["unblock_btn"]))
{
	...
	{
		//for unblocking the selected user
		$result=$con->query("update tbl_signup set block=1 where name='".$_POST['user']."'");
		if($result)
		{
			echo "<script> alert('User unblocked.....'); location.replace(location.href);</script>";
		}
	}
